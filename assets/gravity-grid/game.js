...
var ASM_CONSTS = {
  648544: () => { FS.mkdir('/saves'); FS.mount(IDBFS, {}, '/saves'); FS.syncfs(true, function(err) { assert(!err); Module.ccall('em_fs_init'); }); },  
 648673: () => { FS.syncfs(function(err) { assert(!err); }); },  
 648717: () => { Module.ctx = Module.createContext(Module.canvas, true, true, { antialias: true, alpha : false, premultipliedAlpha : false, }); let mouseUpdate = Module.cwrap('mouseUpdate', 'void', ['number', 'number', 'number', 'number']); let onPointerLeave = Module.cwrap('onPointerLeave', 'void', []); Module.canvas.addEventListener("pointermove", (event) => { mouseUpdate(event.offsetX, event.offsetY, 0, 0); }); Module.canvas.addEventListener("pointerdown", (event) => { let downButtons = Math.pow(2, event.button); mouseUpdate(event.offsetX, event.offsetY, downButtons, 0); }); Module.canvas.addEventListener("pointerup", (event) => { let upButtons = Math.pow(2, event.button); mouseUpdate(event.offsetX, event.offsetY, 0, upButtons); }); Module.canvas.addEventListener("pointerleave", (event) => { onPointerLeave(); }); },  
 649532: () => { return Module.canvas.width; },  
 649564: () => { return Module.canvas.height; },  
 649597: () => { return window.devicePixelRatio; }
};
...
var _em_fs_init = Module['_em_fs_init'] = createExportWrapper('em_fs_init');
var _main = createExportWrapper('main');
var _malloc = createExportWrapper('malloc');
var _free = createExportWrapper('free');
var _fflush = Module['_fflush'] = createExportWrapper('fflush');
var _mouseUpdate = Module['_mouseUpdate'] = createExportWrapper('mouseUpdate');
var _onPointerLeave = Module['_onPointerLeave'] = createExportWrapper('onPointerLeave');
...
var ___emscripten_embedded_file_data = Module['___emscripten_embedded_file_data'] = 270212;
